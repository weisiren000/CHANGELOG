...
- **新增模型**：添加 SiliconFlow DeepSeek-V3.2 模型支持（支持思考模式，可通过 thinking_budget 参数配置）
...
- **DeepSeek-V3.2 模型**：
  ...
  - ✅ 支持思考模式，可通过 `thinking_budget` 参数配置推理预算，不限于工具调用场景
...
- **新模型**：DeepSeek-V3.2 模型具备强大的推理能力，支持通过 `thinking_budget` 参数直接开启思考模式
